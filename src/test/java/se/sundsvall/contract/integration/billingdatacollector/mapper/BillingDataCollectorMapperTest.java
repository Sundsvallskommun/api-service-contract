...
class BillingDataCollectorMapperTest {
	...
	private static Stream<Arguments> toScheduledBillingArgumentProvider() {
		return Stream.of(
	...
	@ParameterizedTest(name = "{0}")
	@MethodSource("toScheduledBillingArgumentProviderYearly")
	void toScheduledBillingYearly(String description, IntervalType interval, LeaseType leaseType, Set<Integer> expectedDays, Set<Integer> expectedMonths) {
		// Arrange
		final var randomId = UUID.randomUUID().toString();
		...
		when(contractEntityMock.getInvoicing()).thenReturn(invoicingEmbeddableMock);
		when(invoicingEmbeddableMock.getInvoiceInterval()).thenReturn(interval);
		when(contractEntityMock.getLeaseType()).thenReturn(leaseType);
		when(contractEntityMock.getCurrentPeriodEndDate()).thenReturn(LocalDate.of(2025, 6, 30));
	...
	private static Stream<Arguments> toScheduledBillingArgumentProviderYearly() {
		return Stream.of(
			Arguments.of("Agreement with yearly billing cycle LAND_LEASE_RESIDENTIAL", YEARLY, LAND_LEASE_RESIDENTIAL, Set.of(1), Set.of(6)),
			Arguments.of("Agreement with yearly billing cycle LAND_LEASE_MISC", YEARLY, LAND_LEASE_MISC, Set.of(1), Set.of(12)));
	}
